...
class PredictionDenseSoftmax(PredictionBasic):
    ...
    def build_graph(self, reps, context_mask):
        ...
        with vs.variable_scope(self.scope): 
            ...
            # The value at each index is max(prob_end(index), prob_end(index+1) ....)
            end_prob_max = tf.identity(probdist_end)
            # The value at each index is max(prob_start(index), prob_start(index-1) ...)
            start_prob_max = tf.identity(probdist_start)

            for index in range(1, self.answer_length):
                padding_at_end = tf.constant([[0, 0], [0, index]])
                padding_at_start = tf.constant([[0, 0], [index, 0]])

                end_prob_max = tf.maximum(end_prob_max,
                                      tf.pad(probdist_end[:, index:],
                                             padding_at_end, "CONSTANT"))
                start_prob_max = tf.maximum(end_prob_max,
                                        tf.pad(probdist_start[:, :-index],
                                               padding_at_start, "CONSTANT"))
            probdist_start = probdist_start * end_prob_max
            probdist_end   = probdist_end * start_prob_max
