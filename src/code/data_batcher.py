...
class Batch(object):
    ...
    def __init__(self, context_ids, context_mask, context_tokens, qn_ids,
        ...
        Inputs:
          ...
          lists (unpadded) of tokens (strings)
...
def padded(token_batch, batch_pad=0):
    ...
    Inputs:
      ...
      batch_pad: Int. Length to pad to. If 0, pad to maximum length sequence in
      token_batch.
    Returns:
    ...
    """
    maxlen = max(map(lambda x: len(x), token_batch)) if batch_pad == 0\
                                                     else batch_pad
    return map(
            lambda token_list: token_list + [PAD_ID] * (maxlen - len(token_list)),
            token_batch)


def refill_batches(batches, word2id, context_file, qn_file, ans_file,
                   batch_size, context_len, question_len, discard_long):
    """
    ...
    Inputs:
      ...
      context_file, qn_file, ans_file: 
      paths to {train/dev}.{context/question/answer} data files
      batch_size: int. how big to make the batches context_len,
      question_len: max length of context and question respectively
      discard_long: 
                  If True, discard any examples that are longer than
    ...
    while context_line and qn_line and ans_line: 
        ...
        # add to examples
        examples.append( (context_ids, context_tokens, qn_ids,
    ...
    # Note: if you sort by context length, then you'll have batches 
    # which contain the same context many times 
    ...
    for batch_start in xrange(0, len(examples), batch_size):
        ...
        batches.append(
                (context_ids_batch, context_tokens_batch, qn_ids_batch,
                 qn_tokens_batch, ans_span_batch, ans_tokens_batch) )
...
def get_batch_generator(word2id, context_path, qn_path, ans_path, batch_size,
    ...
    Read this to understand generators and the yield keyword in Python:
    ...
    while True:
        if len(batches) == 0: # add more batches
            refill_batches(batches, word2id, context_file, qn_file, ans_file,
                           batch_size, context_len, question_len, discard_long)
        if len(batches) == 0:
            break
